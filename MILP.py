...
OUTPUT_DIR = Path("/Users/macbook/Desktop/mip_outputs")
...
APL_CANDIDATE_COUNTS_TO_TEST = [12, 20, 30, 50, None]
...
def generate_apl_candidates(
    ...
    if candidate_count is None:
        selected_rows = [row for _, row in candidates.iterrows()]
    else:
        selected_rows = []
...
def build_distance_matrix(cbs_demand: pd.DataFrame, facilities: List[Facility]) -> np.ndarray:
    cbs_x = cbs_demand["X"].to_numpy()
    cbs_y = cbs_demand["Y"].to_numpy()
    fac_x = np.array([facility.x for facility in facilities])
    fac_y = np.array([facility.y for facility in facilities])
...
def build_cost_matrices(
    ...
    demand = cbs_demand["annual_demand"].to_numpy()
    p_pick = np.minimum(raw_pickup_probability(dist_matrix) * pickup_probability_scale, 0.95)
...
def solve_mip(
    ...
    open_solution = np.array([int(round(pulp.value(y[j]) or 0)) for j in range(n_facilities)])
...
def evaluate_solution_components(
    ...
    p_pick = np.minimum(raw_pickup_probability(assigned_dist) * pickup_probability_scale, 0.95)

    annual_pickups = demand * p_pick
    annual_deliveries = demand * (1 - p_pick)

    fixed_cost = sum(facility.fixed_cost for facility, is_open in zip(facilities, open_solution) if is_open)
    ...
    return {
        ...
        "total_cost": float(fixed_cost + delivery_cost + storage_cost + apl_operating_cost + long_distance_penalty),
...
def run_mip_for_candidate_count(
    sp: pd.DataFrame,
    activity: pd.DataFrame,
    cbs: pd.DataFrame,
    nodes: pd.DataFrame,
    ...
    apl_candidates = generate_apl_candidates(
        cbs_demand,
        existing_facilities,
        candidate_count,
    )
    ...
    print("\n" + "=" * 70)
    print(f"Running MILP with APL candidate count: {candidate_count if candidate_count is not None else 'ALL'}")
    print(f"Existing SPs: {len(existing_facilities)}")
    ...
    summary["apl_candidate_count"] = len(apl_candidates)
    summary["candidate_setting"] = candidate_count if candidate_count is not None else "ALL"
    ...
    print(f"City bounce: {summary['city_bounce_rate']:.3%}")
    print(f"Max location bounce: {summary['max_location_bounce_rate']:.3%}")
...
def main() -> None:
    ...
    for candidate_count in APL_CANDIDATE_COUNTS_TO_TEST:
        summary = run_mip_for_candidate_count(
            sp,
            activity,
            cbs,
            nodes,
    ...
    sensitivity_df.to_csv(OUTPUT_DIR / "mip_apl_candidate_sensitivity.csv", index=False)

    feasible = sensitivity_df[
        (sensitivity_df["solver_status"] == "Optimal")
        & (sensitivity_df["city_bounce_rate"] <= 0.01)
        & (sensitivity_df["max_location_bounce_rate"] <= 0.02)
    ].copy()

    best_setting = feasible.sort_values("total_cost").iloc[0]["candidate_setting"]

    if best_setting == "ALL":
        best_candidate_count = None
    else:
        best_candidate_count = int(best_setting)

    print("\n" + "=" * 70)
    print("Best sensitivity setting")
    print(f"APL candidate setting: {best_setting}")
    print("Re-running best model and saving full outputs...")

    best_summary = run_mip_for_candidate_count(
        sp,
        activity,
        cbs,
        nodes,
    ...
    print("\nBest MILP summary")
    print(f"Total cost: EUR {best_summary['total_cost']:,.0f}")
    print(f"Open locations: {best_summary['open_locations']}")
    print(f"Open SPs: {best_summary['open_service_points']}")
    print(f"Open APLs: {best_summary['open_apls']}")
    print(f"Average distance: {best_summary['avg_distance_km']:.2f} km")
    print(f"City bounce rate: {best_summary['city_bounce_rate']:.3%}")
    print(f"Max location bounce rate: {best_summary['max_location_bounce_rate']:.3%}")
